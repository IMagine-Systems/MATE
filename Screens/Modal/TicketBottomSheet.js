...
const TicketBottomSheet = (props) => {
    ...
    getDoc(myDoc).then((snapshot) => {
      if (snapshot.exists) {
        ...
        if (UserInfo.Driver[0].auth === "driver") {
          for (let i = 0; i < ticket_info.CarpoolTicket.length; i++) {
            if (
              ticket_info.CarpoolTicket[i].student_number ===
                UserInfo.Driver[0].student_number &&
              ticket_info.CarpoolTicket[i].nickname ===
                UserInfo.Driver[0].nickname
            ) {
  ...
  const TicketDelete = () => {
    if (
      data.ticket_name === "카풀" &&
      UserInfo.Driver[0].nickname === data.nickname
    ) {
      const myDoc = doc(db, "CollectionNameCarpoolTicket", "TicketDocument");

      updateDoc(myDoc, {
        CarpoolCount: userDoc.CarpoolCount - 1,
        CarpoolTicket: arrayRemove(data),
      });
      alert("삭제 하였습니다.");
      Read();
      showCarpoolTicket();
  ...
  const addRecruitment = () => {
    ...
    if (data === undefined && data.passenger_info === null) {
      ...
      if (
        ...
        data.nickname != UserInfo.Driver[0].nickname &&
        UserInfo.Driver[0].auth === "driver" &&
        driverTicketOverlap === false
      ) {
        const myDoc = doc(db, "CollectionNameCarpoolTicket", "TicketDocument");
  ...
  const ShowTicketRecruitmentButton = () => {
    if (data === undefined) {
      if (
        default_data.student_number != UserInfo.Driver[0].student_number &&
        default_data.nickname != UserInfo.Driver[0].nickname
      ) {
        return (
      ...
      if (
        data.student_number != UserInfo.Driver[0].student_number &&
        data.nickname != UserInfo.Driver[0].nickname
      ) {
        return (
          <View style={styles.button_container}>
          ...
          <View style={styles.container}>
            ...
            <View style={styles.body}>
              ...
              <View style={styles.driver_info_container}>
                <View>
                  <Text>
                    {data === undefined ? default_data.nickname : data.nickname}
                  </Text>
                  <Text>
                    {data === undefined
                      ? default_data.department
                      : data.department}
                  </Text>
                </View>
              </View>
